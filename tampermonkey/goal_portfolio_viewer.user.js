...
function scrollOverlayContentToTop(sourceNode = null) {
    ...
    const scrollHost = overlay.querySelector('.gpv-body') || overlay.querySelector('.gpv-content');
    if (!scrollHost) {
        return;
    }
    const enforceTop = () => {
    ...
    if (typeof scrollHost.scrollTo === 'function') {
        ...
        if (scrollHost.gpvEnforceTopTimer) {
            clearTimeout(scrollHost.gpvEnforceTopTimer);
        }
        if (typeof requestAnimationFrame === 'function') {
        ...
        scrollHost.gpvEnforceTopTimer = detachTimerHandle(setTimeout(enforceTop, 220));
        return;
    }
    scrollHost.scrollTop = 0;
}
...
// ============================================
    ...
    function buildFsmDriftStatusCardHtml(driftModel, { className = 'gpv-summary-card', dataAttribute = 'data-fsm-card' } = {}) {
        ...
        const detailRows = (driftModel?.portfolios || []).slice(0, 3).map(item => {
            ...
            return `<div class="gpv-fsm-drift-meta">${escapeHtml(item.portfolioLabel)} · ${escapeHtml(driftLabel)} · ${statusLabel}</div>`;
        }).join('');
        const hiddenCount = Math.max(0, (driftModel?.portfolios || []).length - 3);
        const overflowLine = hiddenCount > 0
            ? `<div class="gpv-fsm-drift-meta">+${hiddenCount} more</div>`
            : '';

        return `
            <div class="${escapeHtml(className)}" ${dataAttribute}="fsm-drift-status">
                <div class="gpv-fsm-drift-label">FSM drift status</div>
                <div class="gpv-fsm-drift-value">${escapeHtml(headline)}</div>
                <div class="gpv-fsm-drift-meta">${escapeHtml(coverageMeta)}</div>
                ${detailRows}
                ${overflowLine}
            </div>
        `;
    ...
    function buildFsmWorkspaceDriftPanel(driftModel) {
        const panel = createElement('div', 'gpv-summary-row gpv-fsm-drift-panel');
        panel.innerHTML = buildFsmDriftStatusCardHtml(driftModel, {
            className: 'gpv-summary-card gpv-fsm-drift-card',
            dataAttribute: 'data-workspace-card'
        });
        return panel;
    }
    ...
    function renderFsmWorkspace(contentDiv, fsmHoldings) {
        ...
        const rerender = () => {
            ...
            contentDiv.appendChild(buildFsmWorkspaceDriftPanel(driftModel));
